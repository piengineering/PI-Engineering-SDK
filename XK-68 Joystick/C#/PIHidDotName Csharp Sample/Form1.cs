    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        public void HandlePIEHidData(Byte[] data, PIEDevice sourceDevice, int error)
        {
            ...
            if (sourceDevice == devices[selecteddevice])
            {
                ...
                if (data[2] < 4) //means general incoming data report
                {
                    ...
                    for (int i = 0; i < maxcols; i++)
                    {
                        ...
                        for (int j = 0; j < maxrows; j++)
                        {
                            ...
                            int keynum = 8 * i + j; //using key numbering in sdk; C1=0,1,2 C2=8,9,10 C3=16,17,18 C4=24,25,26
                    ...
                    //Joystick
                    c = LblJoyX;
                    this.SetText("Joy X: " + data[15].ToString());
                    c = LblJoyY;
                    this.SetText("Joy Y: " + data[16].ToString());
                    c = LblJoyZ;
                    this.SetText("Joy Z: " + data[17].ToString());
                    ...
                    if (devices[i].HidUsagePage == 0xc && devices[i].WriteLength>10)
                    {
                        switch (devices[i].Pid)
                        {
                            case 0x045D:
                                ...
                                CboDevices.Items.Add("XK-68 Joystick ("+devices[i].Pid+"=PID #1), ID: " + i);
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 0x045E:
                                //Device 1 Keyboard, Joystick, Mouse and Output endpoints
                                CboDevices.Items.Add("XK-68 Joystick (" + devices[i].Pid + "), ID: " + i);
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 0x045F:
                                //Device 2 Keyboard, Mouse, Input and Output endpoints
                                CboDevices.Items.Add("XK-68 Joystick (" + devices[i].Pid + "=PID #2), ID: " + i);
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            default:

...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_GETBLSTATE: 
			...
			buffer[2]= atoi(keyid2);  //index of desired button
...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		productname="Found Device: XBK-16x6 Module, PID="+pidstring;
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	else if (pData[2] == 146) //0x92 Power Levels Request for RGB LEDs
	{
		//pData[3]=current power MSB of 3 bytes
		//pData[4]=current power middle byte
		//pData[5]=current power LSB of 3 bytes
		//current power = (pData[3] * 65536) + (pData[4] * 256) + pData[5]
		//pData[7]=maximum power MSB of 3 bytes
		//pData[8]=maximum power middle byte
		//pData[9]=maximum power LSB of 3 bytes
		//maximum power = (pData[7] * 65536) + (pData[8] * 256) + pData[9]
		//pData[10]=bank 1 dim factor (255=100%)
		//pData[11]=bank 2 dim factor (255=100%)
	}
	else if ((pData[2] == 165) || (pData[2] == 181)) //0xA5 0r 0xB5 Power Overload from Individual Button RGB Backlight Change
    {
		...
		//pData[3]=bank where 0=bank 1, 1=bank 2, this is the bank that was set when power overload occurred 
		//pData[4]=button user was setting when power overload occurred
		//pData[5]=% reduction of dim factors (100-pData[5])
		//pData[6]=new bank 1 dim factor, the firmware automatically adjusted dim factors when power overload is detected
		//pData[7]=new bank 2 dim factor, the firmware automatically adjusted dim factors when power overload is detected
		//pData[8]=previous bank 1 dim factor
		//pData[9]=previous bank 2 dim factor
	}
	else if ((pData[2] == 166) || (pData[2] == 182)) //0xA6 0r 0xB6 Power Overload from Bank Change
    {
        //Power overload, this message is sent when the total "power" of all illuminated RGB LEDs exceeds the maximum set for the unit
        //The 'power' is simply the sum of the upper bank and lower bank RGB values for all buttons. For example, the factory default setting is all LEDs set to red=3, green=3, and blue=3
        //has a power=1728.  
	...
	else if ((pData[2] == 164) || (pData[2] == 187)) //0xA6 0r 0xB6 Power Overload from Dim Factor Change
    {
        //Power overload, this message is sent when the total "power" of all illuminated RGB LEDs exceeds the maximum set for the unit
        //The 'power' is simply the sum of the upper bank and lower bank RGB values for all buttons. For example, the factory default setting is all LEDs set to red=3, green=3, and blue=3
        //has a power=1728.  
		...
		//pData[8]=previous bank 1 dim factor
		//pData[9]=previous bank 2 dim factor
		//pData[10]=desired bank 1 dim factor, user wanted this factor but it caused overload so automatically adjusted to data[6]
		//pData[11]=desired bank 2 dim factor, user wanted this factor but it caused overload so automatically adjusted to data[7]
	}

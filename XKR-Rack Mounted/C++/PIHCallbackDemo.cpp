	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				if (pid==1279)
				{
					AddDevices(hDialog, "Found Device: XKR-32 Rack Mounted Keys, PID=1279 (PID #1)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1280)
				{
					AddDevices(hDialog, "Found Device: XKR-32 Rack Mounted Keys, PID=1280 (PID #2)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1281)
				{
					AddDevices(hDialog, "Found Device: XKR-32 Rack Mounted Keys, PID=1281 (PID #3)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1282)
				{
					AddDevices(hDialog, "Found Device: XKR-32 Rack Mounted Keys, PID=1282 (PID #4)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else

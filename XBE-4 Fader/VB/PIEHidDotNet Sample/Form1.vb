...
Public Class Form1
    ...
    Public Sub HandlePIEHidData(ByVal data() As Byte, ByVal sourceDevice As PIEHid32Net.PIEDevice, ByVal perror As Integer) Implements PIEHid32Net.PIEDataHandler.HandlePIEHidData
        ...
        If sourceDevice.Pid = devices(selecteddevice).Pid Then
            ...
            If (data(2) < 4) Then 'General Incoming Data
                ...
                'Buttons
                Dim maxcols As Integer = 1 'number of columns of Xkeys digital button data
        ...
        Else
            ...
            For i As Integer = 0 To devices.Length - 1
                ...
                Else
                    ...
                    ElseIf (devices(i).Pid = 1487) Then
    ...
    Private Sub GetFaderPositions()
        If selecteddevice <> -1 Then
            ...
            While (ret = 0 AndAlso data(2) <> 177) OrElse ret = 304
    ...
    Private Sub BtnDescriptor_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles BtnDescriptor.Click
        If selecteddevice <> -1 Then
            ...
            Dim savecallbackstate As Boolean = devices(selecteddevice).callNever 'turn off callback so returned data isn't grabbed there
            devices(selecteddevice).callNever = True
            ...
            Dim ledon As String = ""
            If CByte((ddata(10) And 1)) <> 0 Then ledon = "NumLock, "
            If CByte((ddata(10) And 2)) <> 0 Then ledon = ledon & "CapsLock, "
            If CByte((ddata(10) And 4)) <> 0 Then ledon = ledon & "ScrLock, "
            If ledon = "" Then ledon = "None"
    ...
    Private Sub btnSetAllFaders_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSetAllFaders.Click
        ...
        If selecteddevice <> -1 Then
            ...
            wdata(0) = 0
            wdata(1) = 168 '&HA8
            wdata(2) = pos 'desired position for fader 1
            wdata(3) = pos 'desired position for fader 2
            wdata(4) = pos 'desired position for fader 3
            wdata(5) = pos 'desired position for fader 4
    ...
    Private Sub btnGetCapSens_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGetCapSens.Click
        ...
        If selecteddevice <> -1 Then
            ...
            listBox4.Items.Add("Fader 1 = " + (ddata(3) * 256 + ddata(4)).ToString)
            listBox4.Items.Add("Fader 2 = " + (ddata(5) * 256 + ddata(6)).ToString)
            listBox4.Items.Add("Fader 3 = " + (ddata(7) * 256 + ddata(8)).ToString)
            listBox4.Items.Add("Fader 4 = " + (ddata(9) * 256 + ddata(10)).ToString)
    ...
    Private Sub tbFader1_Scroll(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tbFader1.Scroll, tbFader4.Scroll, tbFader3.Scroll, tbFader2.Scroll
        ...
        If selecteddevice <> -1 Then
            ...
            wdata(0) = 0
            wdata(1) = 149 '&H95
            wdata(2) = faderindex '0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4
    ...
    Private Sub tbFader1_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tbFader1.ValueChanged, tbFader4.ValueChanged, tbFader3.ValueChanged, tbFader2.ValueChanged
        Dim thistrackbarr As TrackBar = CType(sender, TrackBar)
    ...
    Private Sub btnGetTouchStop_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnGetTouchStop.Click
        ...
        If selecteddevice <> -1 Then
            ...
            If result <> 0 Then
                LblStatus.Text = "Write Fail: " + result.ToString
            Else
                LblStatus.Text = "Write Success - Get fader touch stop setting"
            End If

            'after this write the next read with 3rd byte = 162 gives the touch stop setting data
            Dim ddata(devices(selecteddevice).ReadLength) As Byte
            Dim countout As Integer = 0
            result = devices(selecteddevice).BlockingReadData(ddata, 100)
            ...
            End While
            listBox4.Items.Clear()
            listBox4.Items.Add("Fader 1 touch stop=" + ddata(3).ToString())
            listBox4.Items.Add("Fader 2 touch stop=" + ddata(4).ToString())
            listBox4.Items.Add("Fader 3 touch stop=" + ddata(5).ToString())
            listBox4.Items.Add("Fader 4 touch stop=" + ddata(6).ToString())

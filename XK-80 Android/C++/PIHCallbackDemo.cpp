...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1217)
				{
					AddEventMsg(hDialog, "Found Device: XK-80 Android, PID=1217 (PID #1)");
					hDevice = hnd;
				}
				else if (pid==1218)
				{
					AddEventMsg(hDialog, "Found Device: XK-80 Android, PID=1218 (PID #2)");
					hDevice = hnd;
				}
				else if (pid==1219)
				{
					AddEventMsg(hDialog, "Found Device: XK-80 Android, PID=1219 (PID #3)");
					hDevice = hnd;
				}
				else if (pid==1220)
				{
					AddEventMsg(hDialog, "Found Device: XK-80 Android, PID=1220 (PID #4)");
					hDevice = hnd;
				}
				if (pid==1231)
				{
					AddEventMsg(hDialog, "Found Device: XK-60 Android, PID=1231 (PID #1)");
					hDevice = hnd;
				}
				else if (pid==1232)
				{
					AddEventMsg(hDialog, "Found Device: XK-60 Android, PID=1232 (PID #2)");
					hDevice = hnd;
				}
				else if (pid==1233)
				{
					AddEventMsg(hDialog, "Found Device: XK-60 Android, PID=1233 (PID #3)");
					hDevice = hnd;
				}
				else if (pid==1234)
				{
					AddEventMsg(hDialog, "Found Device: XK-60 Android, PID=1234 (PID #4)");
					hDevice = hnd;
				}
				if (hDevice!=-1)
				{
	...
	AddEventMsg(hDialog, "No X-keys XK-80/60 Android devices found");
}
//------------------------------------------------------------------------

            ...
            For i As Integer = 0 To devices.Length - 1
                ...
                If devices(i).HidUsagePage = 12 And devices(i).WriteLength = 36 Then
                    ...
                    Select Case devices(i).Pid
                        Case 1300
                            CboDevices.Items.Add("XK-3 Switch Interface KVM(" + devices(i).Pid.ToString + "=PID #1): " + i.ToString)
                            cbotodevice(cbocount) = i
                            cbocount = cbocount + 1
                        Case 1302
                            CboDevices.Items.Add("XK-12 Switch Interface KVM(" + devices(i).Pid.ToString + "=PID #1): " + i.ToString)
                            cbotodevice(cbocount) = i
                            cbocount = cbocount + 1
                        Case Else
                    ...
                    EnableAllControls()
                Else
                    If (devices(i).Pid = 1301) Then
                        CboDevices.Items.Add("XK-3 Switch Interface KVM (" + devices(i).Pid.ToString + "=PID #2)")
                        cbotodevice(cbocount) = i
                        cbocount = cbocount + 1
                        DisableAllControls()
                        MessageBox.Show("Device in PID #2 (KVM), no input or output reports are available. To exit KVM replug device into usb port and immediately after press Scroll Lock 10-15 times.")

                    ElseIf (devices(i).Pid = 1303) Then
                        CboDevices.Items.Add("XK-12 Switch Interface KVM (" + devices(i).Pid.ToString + "=PID #2)")
                        cbotodevice(cbocount) = i
                        cbocount = cbocount + 1
                        DisableAllControls()
                        MessageBox.Show("Device in PID #2 (KVM), no input or output reports are available.  To change to PID #1 replug device into usb port and immediately after press Scroll Lock 10-15 times.")

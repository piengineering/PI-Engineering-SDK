...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
        ...
        case IDC_CHKBLONOFF:
            ...
            //  6   14  22  na  na  na  na  62  70  78
            //  7   15  23  na  na  na  na  63  71  79

			//Bank 2
			//Add 80 to bank 1 index get corresponding bank 2 index
		...
		case IDC_BL3:
			//Steps through 10 intensity levels
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
			//for this demo turn on the bank 1 backlights
			buffer[0]=0;
			buffer[1]=182;
			buffer[2]=0; //bank, 0=bank1, 1=bank2
			buffer[3]=255; 
			result=404;
			while (result==404)
			{
			...
			buffer[1]=173;
			buffer[2]=0; //bank, 0=bank1, 1=bank2
			buffer[3]=1; //0=decrease, 1=increase
			buffer[4]=0; //0=wrap, 1=no wrap
		...
		case IDC_DESCRIPTOR:
			...
			if (result ==0 && buffer[2]==214)
			{
				...
				strcpy_s (str,"Sizeofeeprom MSB ");
				strcat_s (str,dataStr);
				AddEventMsg(hDialog, str);

				_itoa_s(buffer[7],dataStr,10);
				strcpy_s (str,"Sizeofeeprom LSB ");
				strcat_s (str,dataStr);
				AddEventMsg(hDialog, str);
...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			hnd = info[i].Handle; //handle required for piehid32.dll calls, i=Handle
			...
			else    
			{
				if (pid==1114)
				{
					AddDevices(hDialog, "Found Device: XK-68 Jog & Shuttle, PID=1114 or PID #1");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1115)
				{
					AddDevices(hDialog, "Found Device: XK-68 Jog & Shuttle, PID=1115");
					combotodevice[cbocount] = i;
					cbocount++;
				}
				else if (pid==1116)
				{
					AddDevices(hDialog, "Found Device: XK-68 Jog & Shuttle, PID=1116 or PID #2");
					combotodevice[cbocount] = i;
					cbocount++;
				}
				else
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
		...
		//pData[0] always 0
		//pData[1] unit ID
		//pData[2] program switch and data type
		//pData[3] 1st column of buttons
		//pData[4] 2nd column of buttons
		//pData[5] 3rd column of buttons
		//pData[6] 4th column of buttons (bits 1-5=buttons, bit 8=Jog Knob clockwise)
		//pData[7] 5th column of buttons (bits 1-5=buttons, bit 8=Jog Knob counter clockwise)
		//pData[8] 6th column of buttons (bits 1-5=buttons, bit 8=Shuttle at rest)
		//pData[9] 7th column of buttons
		//pData[10] 8th column of buttons
		//pData[11] 9th column of buttons
		//pData[12] 10th column of buttons
		//pData[13] Special
		//pData[14] na
		//pData[15] Shuttle digital clockwise
		//pData[16] Shuttle digital counter clockwise
		//pData[17] Jog analog
		//pData[18] Shuttle analog
		//pData[19] Time Stamp MSB
		//pData[20] Time Stamp 
		//pData[21] Time Stamp 
		//pData[22] Time Stamp LSB

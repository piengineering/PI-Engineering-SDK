...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				if (pid==1180)
				{
					AddEventMsg(hDialog, "Found Device: XK-24 Android, PID=1180 (PID #1)");
					hDevice = hnd;
				}
				else if (pid==1181)
				{
					AddEventMsg(hDialog, "Found Device: XK-24 Android, PID=1181 (PID #2)");
					hDevice = hnd;
				}
				else if (pid==1182)
				{
					AddEventMsg(hDialog, "Found Device: XK-24 Android, PID=1182 (PID #3)");
					hDevice = hnd;
				}
				else if (pid==1183)
				{
				...
				if (hDevice!=-1)
				{
					...
					readlength=info[hDevice].readSize; 
					return;
				}
			}
	...
	AddEventMsg(hDialog, "No XK-24 Android found");
}
//------------------------------------------------------------------------
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	AddEventMsg(hDialog, dataStr);
		...
		for (int i=0;i<maxcols;i++) //loop for each column of button data (Max Cols)
		{
			...
			{
				int temp1=pow(2.0,j);
				int keynum=maxrows*i+j+1; //add 1 to make 1 based indexing instead of 0 based
				char ctemp[10];

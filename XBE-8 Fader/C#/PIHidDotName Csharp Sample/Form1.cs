    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        public void HandlePIEHidData(Byte[] data, PIEDevice sourceDevice, int error)
        {
            ...
            if (sourceDevice == devices[selecteddevice])
            {
                ...
                if (data[2] < 4) //general incoming data
                {
                    ...
                    int maxcols = 2; //number of columns of Xkeys digital button data, labeled "Keys" in P.I. Engineering SDK - General Incoming Data Input Report
                    ...
                    for (int i = 0; i < maxcols; i++) //loop through digital button bytes 
                    {
                        for (int j = 0; j < maxrows; j++) //loop through each bit in the button byte
                        {
                            ...
                            string[] stringmap = new string[24]; //map the bits to the to index given in SDK documentation
                            
                            //column 1
                            stringmap[0] = "button 0"; stringmap[1] = "button 1"; stringmap[2] = "button 2"; stringmap[3] = "button 3";
                            stringmap[4] = "fader 0 touch"; stringmap[5] = "fader 1 touch"; stringmap[6] = "fader 2 touch"; stringmap[7] = "fader 3 touch";
                            stringmap[8] = "button 5"; stringmap[9] = "button 5"; stringmap[10] = "button 6"; stringmap[11] = "button 7";
                            stringmap[12] = "fader 4 touch"; stringmap[13] = "fader 5 touch"; stringmap[14] = "fader 6 touch"; stringmap[15] = "fader 7 touch";
                           
                            switch (state)
                            {
                                case 1: //key was up and now is pressed
                                    buttonsdown = buttonsdown + stringmap[bitkeynum] + " ";
                                    c = this.lblButtons;
                                    SetText(buttonsdown);
                                    break;
                            ...
                            switch (bitkeynum)
                            {
                                ...
                                case 6: // (fader touched)
                                    ...
                                    break;
                                case 7: //(rightmost fader touched)
                                    if (state == 1) //key was pressed
                                    {
                                        //do press actions
                                        //turn backcolor to Yellow
                                ...
                                case 12: 
                                    ...
                                    break;
                                case 13: //button 5 (fader touched)
                                    if (state == 1) //key was pressed
                                    {
                                        //do press actions
                                        //turn backcolor to Yellow
                                    ...
                                    break;
                                case 14: //button 6 (fader touched)
                                    if (state == 1) //key was pressed
                                    {
                                        //do press actions
                                        //turn backcolor to Yellow
                                    ...
                                    break;
                                case 15: //button 7 (rightmost fader touched)
                                    if (state == 1) //key was pressed
                                    {
                                        //do press actions
                                        //turn backcolor to Yellow
                    ...
                    //Virtual buttons are in data[6]
                    maxcols = 1; //1 byte of virtual buttons
                    maxrows = 8; //number of rows of digital button data
                    int virtualbuttonsbyteoffset = 6; //Virtual buttons are in data[6]
                    ...
                    c = this.lblFader4;
                    this.SetText(data[10].ToString());

                    c = this.lblFader5;
                    this.SetText(data[11].ToString());

                    c = this.lblFader6;
                    this.SetText(data[12].ToString());

                    c = this.lblFader7;
                    this.SetText(data[13].ToString());

                    c = this.lblFader8;
                    this.SetText(data[14].ToString());
                    ...
                    float voltage = (data[15]*256+data[16])/1000.0f;
                    c = lblVoltage;
        ...
        private void BtnCallback_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1)
            {
                for (int i = 0; i < CboDevices.Items.Count; i++)
                {
                    ...
                    BtnGetDataNow_Click(this, null); //get initial states
                }
        ...
        private void tbFader1_Scroll(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                wData[0] = 0;
                wData[1] = 149; //0x95
                wData[2] = (byte)(faderindex); //0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4, 4=fader 5, 5=fader 6, 6=fader 7, 7=fader 8
        ...
        private void btnSetAllFaders_Click(object sender, EventArgs e)
        {
            ...
            wData[0] = 0;
            wData[1] = 168; //0xa8
            wData[2] = (byte)pos; //desired position for fader 0
            wData[3] = (byte)pos; //desired position for fader 1
            wData[4] = (byte)pos; //desired position for fader 2
            wData[5] = (byte)pos; //desired position for fader 3
            wData[6] = (byte)pos; //desired position for fader 4
            wData[7] = (byte)pos; //desired position for fader 5
            wData[8] = (byte)pos; //desired position for fader 6
            wData[9] = (byte)pos; //desired position for fader 7
            int result = 404;
        ...
        {
            //Enables the touch stop feature. If enabled then faders, when being programmatically driven to a position, will stop when user touches fader, allowing user to override the fader position.
            //If disabled then fader will not be able to be moved by the user, it will bounce back to its programmed position.
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                for (int j = 0; j < devices[selecteddevice].WriteLength; j++)
                {
                ...
                wData[0] = 0;
                wData[1] = 161; // 0xA1
                wData[2] = 0; //0=fader 0, 1=fader 1, 2=fader 2 3=fader 3, 4=fader 4, 5=fader 5, 6=fader 6, 7=fader 7 OR 255 to set all
        ...
        {
            //Disables the touch stop feature. If enabled then faders, when being programmatically driven to a position, will stop when user touches fader, allowing user to override the fader position.
            //If disabled then fader will not be able to be moved by the user, it will bounce back to its programmed position.
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                for (int j = 0; j < devices[selecteddevice].WriteLength; j++)
        ...
        private void btnSetCapSens_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                wData[0] = 0;
                wData[1] = 152; //0x98
                wData[2] = (byte)fader; //0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4, 4=fader 5, 5=fader 6, 6=fader 7, 7=fader 8 or 255 for all
        ...
        private void btnGetCapSens_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1)
            {
                ...
                listBox4.Items.Add("Fader 0 touch sensitivity=" + (data[3] * 256 + data[4]).ToString());
                listBox4.Items.Add("Fader 1 touch sensitivity=" + (data[5] * 256 + data[6]).ToString());
                listBox4.Items.Add("Fader 2 touch sensitivity=" + (data[7] * 256 + data[8]).ToString());
                listBox4.Items.Add("Fader 3 touch sensitivity=" + (data[9] * 256 + data[10]).ToString());
                listBox4.Items.Add("Fader 4 touch sensitivity=" + (data[11] * 256 + data[12]).ToString());
                listBox4.Items.Add("Fader 5 touch sensitivity=" + (data[13] * 256 + data[14]).ToString());
                listBox4.Items.Add("Fader 6 touch sensitivity=" + (data[15] * 256 + data[16]).ToString());
                listBox4.Items.Add("Fader 7 touch sensitivity=" + (data[17] * 256 + data[18]).ToString());
        ...
        private void btnXkeysDecrypt_Click(object sender, EventArgs e)
        {
            if (selecteddevice != -1) //do nothing if not enumerated
            {
                ...
                if (decryptthis == "encrypt result")
                {
                    //probably callback was off and never got result 
        ...
        private void btnGetTouchStop_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1)
            {
                ...
                {
                    toolStripStatusLabel1.Text = "Write Success - Get touch stop";
                }
                ...
                data[1] = 0;
                int ret = devices[selecteddevice].BlockingReadData(ref data, 100);
                while ((ret == 0 && data[2] != 162) || ret == 304)
                {
                    if (ret == 304)
                    {
                        // Didn't get any data for 100ms, increment the countout extra
                ...
                listBox4.Items.Clear();
                listBox4.Items.Add("Fader 0 touch stop=" + data[3].ToString());
                listBox4.Items.Add("Fader 1 touch stop=" + data[4].ToString());
                listBox4.Items.Add("Fader 2 touch stop =" + data[5].ToString());
                listBox4.Items.Add("Fader 3 touch stop=" + data[6].ToString());
                listBox4.Items.Add("Fader 4 touch stop=" + data[7].ToString());
                listBox4.Items.Add("Fader 5 touch stop=" + data[8].ToString());
                listBox4.Items.Add("Fader 6 touch stop =" + data[9].ToString());
                listBox4.Items.Add("Fader 7 touch stop=" + data[10].ToString());

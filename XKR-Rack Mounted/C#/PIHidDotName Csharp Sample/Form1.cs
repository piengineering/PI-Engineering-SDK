    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
            ...
            else
            {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc && devices[i].WriteLength==36)
                    {
                        ...
                        {
                            case 1279:
                                CboDevices.Items.Add("XKR-32 Rack Mounted Keys  (" + devices[i].Pid + "=PID #1)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1280:
                                CboDevices.Items.Add("XKR-32 Rack Mounted Keys (" + devices[i].Pid + "=PID #2)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1281:
                                CboDevices.Items.Add("XKR-32 Rack Mounted Keys (" + devices[i].Pid + "=PID #3)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1282:
                                CboDevices.Items.Add("XKR-32 Rack Mounted Keys (" + devices[i].Pid + "=PID #4)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            default:
        ...
        private void ChkFlash_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1)
            {
                ...
                {
                    iindex = Convert.ToInt16(sindex) + 32;  //Add to get corresponding bank 2 index
                }
        ...
        private void BtnIncIntensity_Click(object sender, EventArgs e)
        {
            if (CboDevices.SelectedIndex != -1) //do nothing if not enumerated
            {

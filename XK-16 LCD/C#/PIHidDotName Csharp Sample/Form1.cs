    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        public void HandlePIEHidData(Byte[] data, PIEDevice sourceDevice, int error)
        {
            ...
            if (sourceDevice == devices[selecteddevice])
            {
                ...
                for (int j = 0; j < maxrows; j++)
                {
                    ...
                    {
                        int keynum = 4 * j + i; //using key numbering in sdk; column 1 = 0,1,2... column 2 = 8,9,10... column 3 = 16,17,18... column 4 = 24,25,26... etc
            ...
            else
            {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        else if (devices[i].Pid == 1323)
                        {
                            //Device 7 Keyboard only endpoint
                            CboDevices.Items.Add("XK-16 LCD for KVM (" + devices[i].Pid + "=PID #8), ID: " + i);
                            cbotodevice[cbocount] = i;
                            cbocount++;
        ...
        private void ChkBLOnOff_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1)
            {
                ...
                else //bank 2 backlight
                {
                    ...
                    iindex = Convert.ToInt16(sindex) + 16;  //Add 16 to get corresponding bank 2 index
                }
        ...
        private void ChkFlash_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1)
            {
                ...
                {
                    sindex = sindex.Remove(sindex.IndexOf("-b2"), 3);
                    iindex = Convert.ToInt16(sindex) + 16;  //Add 16 to get corresponding bank 2 backlight
                }
        ...
        {
            for (int i = 0; i < devices[selecteddevice].WriteLength; i++)
            {
                wData[i] = 32; //this is a blank space
            }

            //write line
            string message = txtLCD1.Text;
            wData[0] = 0;
            wData[1] = 0xce;
            wData[2] = 0; //row; 0=row 1, 1=row 2
            wData[3] = 0; //backlight state; 0=off, 1=on

            if (chkBacklight.Checked == true) wData[3] = 1;
            //bytes 4-19 are the 16 characters to print to the row from left to right, ascii values
            for (int i = 0; i < message.Length; i++)
            {
                wData[i+4] = (Byte)message[i];
            }
        ...
        private void btnLCD2_Click(object sender, EventArgs e)
        {
            //line 2
            for (int i = 0; i < devices[selecteddevice].WriteLength; i++)
            {

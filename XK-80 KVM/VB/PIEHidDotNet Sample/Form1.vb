            ...
            For i As Integer = 0 To devices.Length - 1
                If devices(i).HidUsagePage = 12 And devices(i).WriteLength = 36 Then
                    Select Case devices(i).Pid
                        Case 1237
                            CboDevices.Items.Add("XK-80 KVM (" + devices(i).Pid.ToString + "=PID #1): " + i.ToString)
                            cbotodevice(cbocount) = i
                            cbocount = cbocount + 1
                            EnableAllControls()
                        Case 1239
                            CboDevices.Items.Add("XK-60 KVM (" + devices(i).Pid.ToString + "=PID #1): " + i.ToString)
                            cbotodevice(cbocount) = i
                            cbocount = cbocount + 1
                            EnableAllControls()
                        Case Else
                ...
                Else
                    ...
                    ElseIf (devices(i).Pid = 1240) Then
                        'Device 2 Keyboard only endpoint
                        CboDevices.Items.Add("XK-60 KVM (" + devices(i).Pid.ToString + "=PID #1): " + i.ToString)
                        cbotodevice(cbocount) = i
                        cbocount = cbocount + 1

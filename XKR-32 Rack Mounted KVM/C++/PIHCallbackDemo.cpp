...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_SETKEY: //Do not change PID on reboot
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
		...
		case IDC_SETKEY2: //Always change to PID #2 (KVM) on reboot
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    
	{
		...
		int temp=info[i].Usage;
		int temp2=info[i].UP;
		if ((info[i].UP == 0xC) && (pid==1283))    
		{	
			hnd = info[i].Handle;
			...
			{
				hDevice = hnd;
				AddEventMsg(hDialog, "Found Device: X-keys XKR-32 Rack Mount KVM, PID=1283 or PID #1");
				return;
			}
		}
		else
		{
			if (pid==1284)
			{
				AddEventMsg(hDialog, "Found Device: X-keys XKR-32 Rack Mount KVM, PID=1284 or PID #2 (KVM mode).");
	...
	AddEventMsg(hDialog, "No X-keys XKR-32 Rack Mount KVM device found");
}
//------------------------------------------------------------------------

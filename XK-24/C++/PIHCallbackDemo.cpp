	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				...
				{
					AddDevices(hDialog, "Found Device: XK-24, PID=1028 (PID #2)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1027)
				...
				{
					AddDevices(hDialog, "Found Device: XK-24, PID=1249 (PID #4)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	for (int i=0;i<maxcols;i++) //loop for each column of button data (Max Cols)
	{
		...
		{
			int temp1=pow(2.0,j);
			int keynum=maxrows*i+j+1; //add 1 to make 1 based indexing, not 0 based index
			char ctemp[10];

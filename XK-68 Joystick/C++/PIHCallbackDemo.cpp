	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				if (pid==1117)
				{
					AddDevices(hDialog, "Found Device: XK-68 Joystick, PID=1117 or PID #1");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1118)
				{
					AddDevices(hDialog, "Found Device: XK-68 Joystick, PID=1118");
				}
				else if (pid==1119)
				{
					AddDevices(hDialog, "Found Device: XK-68 Joystick, PID=1119 or PID #2");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
		...
		char dataStr[256];
		sprintf_s(dataStr, "%02x : %02x %02x %02x -- %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x -- %02x %02x -- %02x %02x %02x -- %02x %02x %02x %02x\n", 

...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_ZAxis:
		{
			//Sending this command will cause the joystick twist motion to send game controller Z Axis messages
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
			buffer[0]=0;
			buffer[1]=149; //0x95
			buffer[2]=0; //0=send Z Axis (factory default), 1=send Z Rot
		...
		case IDC_ZRot:
		{
			//Sending this command will cause the joystick twist motion to send game controller Z Rotation messages
			for (int i=0;i<wlen;i++)
			{
				buffer[i]=0;
			}
			buffer[0]=0;
			buffer[1]=149; //0x95
			buffer[2]=1; //0=send Z Axis (factory default), 1=send Z Rot
		...
		case IDC_RotateUnit:
		{
			...
			buffer[0]=0;
			buffer[1]=151; //0x97
			buffer[2]=1; //0=0deg (button top, joystick bottom), 1=90deg (joystick on right, buttons on left), 2=180deg (joystick top, buttons bottom), 3=270deg (joystick on left, buttons on right)
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	if (pData[2]<4) //General Incoming Data
	{
		...
		int maxrows=2; //constant, 8 bits per byte
		...
		if ((pData[3] & 0x04)!=0)
		{
			hList = GetDlgItem(hDialog, IDC_JoystickButton);
			char msg[100]="Joystick Button on";
			SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)msg);
		}
		else
		{
			hList = GetDlgItem(hDialog, IDC_JoystickButton);
			char msg[100]="Joystick Button off";
			SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)msg);
		}

		//Joystick Motion (hexadecimal values shown)
		pData[9]=0x50;
		pData[10]=0x51;
		pData[11]=0x52;
		hList = GetDlgItem(hDialog, IDC_JoystickX);
		if (hList == NULL) return TRUE;
		char msg[100];
		char str[80];
		_itoa_s(pData[9],msg,16);
		strcpy_s (str,"X: ");
		strcat_s (str,msg);
		SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)str);

		hList = GetDlgItem(hDialog, IDC_JoystickY);
		if (hList == NULL) return TRUE;
		_itoa_s(pData[10],msg,16);
		strcpy_s (str,"Y: ");
		strcat_s (str,msg);
		SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)str);

		hList = GetDlgItem(hDialog, IDC_JoystickZ);
		if (hList == NULL) return TRUE;
		_itoa_s(pData[11],msg,16);
		strcpy_s (str,"Z: ");
		strcat_s (str,msg);
		SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)str);

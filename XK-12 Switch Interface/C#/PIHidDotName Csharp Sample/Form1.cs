    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
            ...
            else
            {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc && devices[i].WriteLength==36)
                    {
                        ...
                        {
                            case 1192:
                                CboDevices.Items.Add("XK-12 Switch Interface (" + devices[i].Pid + "=PID #1)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1193:
                                CboDevices.Items.Add("XK-12 Switch Interface (" + devices[i].Pid + "=PID #2)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1194:
                                CboDevices.Items.Add("XK-12 Switch Interface (" + devices[i].Pid + "=PID #3)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1195:
                                CboDevices.Items.Add("XK-12 Switch Interface (" + devices[i].Pid + "=PID #4)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1221:
                                CboDevices.Items.Add("XK-3 Switch Interface (" + devices[i].Pid + "=PID #1)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1222:
                                CboDevices.Items.Add("XK-3 Switch Interface (" + devices[i].Pid + "=PID #2)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1223:
                                CboDevices.Items.Add("XK-3 Switch Interface (" + devices[i].Pid + "=PID #3)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1224:
                                CboDevices.Items.Add("XK-3 Switch Interface (" + devices[i].Pid + "=PID #4)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            default:
        ...
        private void BtnJoyreflect_Click(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1) //do nothing if not enumerated
            {
                ...
                wData[7] = Convert.ToByte(TxtJoyGame1.Text); //buttons 1-8, where bit 1 is button 1, bit 2 is button 2, etc.
        ...
        private void BtnMousereflect_Click(object sender, EventArgs e)
        {
            if (CboDevices.SelectedIndex != -1) //do nothing if not enumerated
            {
                ...
                wData[3] = Convert.ToByte(TxtMouseX.Text); //Mouse X motion. 0 no motion, 1-127 is right, 255-129=left, finest inc (1 and 255) to coarsest (127 and 129).
                wData[4] = Convert.ToByte(TxtMouseY.Text); //Mouse Y motion. 0 no motion, 1-127 is down, 255-129=up, finest inc (1 and 255) to coarsest (127 and 129).
                wData[5] = Convert.ToByte(TxtMouseWheel.Text);//Wheel Y. 0 no motion, 1-127 is up, 255-129=down, finest inc (1 and 255) to coarsest (127 and 129).
                int result = 404;
                while (result == 404) { result = devices[selecteddevice].WriteData(wData); }

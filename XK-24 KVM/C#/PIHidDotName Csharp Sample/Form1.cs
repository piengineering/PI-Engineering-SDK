    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        private void BtnEnumerate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc && devices[i].WriteLength>10)
                    {
                        switch (devices[i].Pid)
                        {
                            case 1235:
                                ...
                                CboDevices.Items.Add("XK-24 KVM (" + devices[i].Pid + "=PID #1), ID: " + i);
                    ...
                    {
                        if (devices[i].Pid == 1236)
                        {
                            //Device 1 Keyboard only endpoint
                            CboDevices.Items.Add("XK-24 KVM (" + devices[i].Pid + "=PID #2), ID: " + i);
                            cbotodevice[cbocount] = i;
                            cbocount++;
                            DisableAllControls();
        ...
        private void BtnDescriptor_Click(object sender, EventArgs e)
        {
            ...
            if (selecteddevice != -1)
            {
                ...
                else if (data[3] == 1) listBox2.Items.Add("PID #1"); //0=PID #1, 1=PID #2

    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
            ...
            else
            {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    if (devices[i].HidUsagePage == 0xc && devices[i].WriteLength==36)
                    {
                        ...
                        {
                            case 1196:
                                CboDevices.Items.Add("Matrix Android (" + devices[i].Pid + "=PID #1)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1197:
                                CboDevices.Items.Add("Matrix Android (" + devices[i].Pid + "=PID #2)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1198:
                                CboDevices.Items.Add("Matrix Android (" + devices[i].Pid + "=PID #3)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1199:
                                CboDevices.Items.Add("Matrix Android(" + devices[i].Pid + "=PID #4)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1200:
                                CboDevices.Items.Add("Footpedal Android (" + devices[i].Pid + "=PID #1)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1201:
                                CboDevices.Items.Add("Footpedal Android (" + devices[i].Pid + "=PID #2)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1202:
                                CboDevices.Items.Add("Footpedal Android (" + devices[i].Pid + "=PID #3)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            case 1203:
                                CboDevices.Items.Add("Footpedal Android (" + devices[i].Pid + "=PID #4)");
                                cbotodevice[cbocount] = i;
                                cbocount++;
                                break;
                            default:
        ...
        private void BtnDescriptor_Click(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1 && devices[selecteddevice].ReadLength>0)
            {
                ...
                temp = "Max Address Written=" + (data[16] * 256 + data[15]).ToString();
                listBox2.Items.Add(temp);
            }
        }

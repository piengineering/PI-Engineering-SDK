                     ...
                     int       nCmdShow)
{
	...
	hList = GetDlgItem(hDialog, IDC_TxtRow1);
	if (hList == NULL) return 0;
	SendMessage(hList, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)"row 1");

	hList = GetDlgItem(hDialog, IDC_TxtRow2);
	if (hList == NULL) return 0;
	SendMessage(hList, WM_SETTEXT, 0, (LPARAM)(LPCTSTR)"row 2");

	hList = GetDlgItem(hDialog, IDC_ChkLCDBacklight);
	if (hList == NULL) return 0;
	SendMessage(hList, BM_SETCHECK, BST_CHECKED, NULL);

	Pid_Combo = CreateWindowEx(0, "ComboBox", NULL, WS_VISIBLE | WS_CHILD | CBS_DROPDOWNLIST, 435, 665, 489, 607, hDialog, NULL, hInstance, NULL); 
...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_BtnWriteLCD1:
			...
			buffer[0]=0;
			buffer[1]=206; //0xce
			buffer[2]=0; //row; 0=row 1, 1=row 2
			buffer[3]=0; //backlight state; 0=off, 1=on
			...
			char row1[16];
			hList = GetDlgItem(hDialog, IDC_TxtRow1);
			SendMessage(hList, WM_GETTEXT, 16, (LPARAM)row1);
			
			for (int i=4;i<20;i++) {buffer[i]=32;} //fill all characters with spaces
			for (int i=0;i<16;i++)
			{
				buffer[i+4]=row1[i];
			}
		...
		case IDC_BtnWriteLCD2:
			...
			buffer[0]=0;
			buffer[1]=206; //0xce
			buffer[2]=1; //row; 0=row 1, 1=row 2
			buffer[3]=0; //backlight state; 0=off, 1=on
			//Get the Backlight state
			hList = GetDlgItem(hDialog, IDC_ChkLCDBacklight);
			if (SendMessage(hList, BM_GETCHECK, 0, 0)==BST_CHECKED) buffer[3]=1;
			...
			char row2[16];
			hList = GetDlgItem(hDialog, IDC_TxtRow2);
			SendMessage(hList, WM_GETTEXT, 16, (LPARAM)row2);
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				if (pid==1316)
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1316 (PID #1)");
				...
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1317 (PID #2)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
					pidlist=1;
				}
				...
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1318 (PID #3)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
					pidlist=2;
				}
				...
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1319 (PID #4)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
					pidlist=3;
				}
				...
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1320 (PID #5)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
					pidlist=4;
				}
				...
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1321 (PID #6)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
					pidlist=5;
				}
				...
				{
					AddDevices(hDialog, "Found Device: XK-16 LCD, PID=1322 (PID #7)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
					pidlist=6;
				}
		...
		else
		{
			if (pid==1323)
			{
				AddDevices(hDialog, "Found Device: X-keys XK-16 LCD, PID=1323 or PID #8 (KVM).");

	...
	for (long i=0; i<count; i++)    
	{
		...
		{	
			...
			else
			{
				if (pid==1300)
				{
					AddDevices(hDialog, "Found Device: XK-3 Switch Interface KVM, PID=1300 (PID #1)");
				}
				else
				{
		...
		{
			AddDevices(hDialog, "Found Device: XK-3 Switch Interface KVM, PID=1301 or PID #2 (KVM)");	
			AddDevices(hDialog, "No input or output reports are available.  To change to PID #1");
			AddDevices(hDialog, "replug device into usb port and quickly press Scroll Lock 10-15 times.");	
			return;
		}

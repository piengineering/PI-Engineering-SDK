...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_CHECK1:
			...
			buffer[1]=179; //0xb3
			buffer[2]=0; //0=LED1, 1=LED2, 2=LED3, 3=LED4
		...
		case IDC_CHECK2:
			...
			buffer[1]=179; //0xb3
			buffer[2]=1; //0=LED1, 1=LED2, 2=LED3, 3=LED4
			buffer[3]=0; //state, 0=off, 1=on, 2=flash
			//get checked state
		...
		case IDC_CHECK3:
			...
			buffer[1]=179; //0xb3
			buffer[2]=2; //0=LED1, 1=LED2, 2=LED3, 3=LED4
			buffer[3]=0; //state, 0=off, 1=on, 2=flash
			//get checked state
		...
		case IDC_CHECK4:
			...
			buffer[1]=179; //0xb3
			buffer[2]=3; //0=LED1, 1=LED2, 2=LED3, 3=LED4
			buffer[3]=0; //state, 0=off, 1=on, 2=flash
			//get checked state
		...
		case IDC_SetResolution:
			//Set mouse resolution CPI
			//Resolution Index
            //1=400
            //2=600
            //3=800
            //4=1000
            //5=1100 (factory default)
            //6=1200
            //7=1300
            //8=1400
            //9=1500
		...
		case IDC_SetCLDelay:
			{
			//ClickLock Delay is between 200 and 2200 ms, 1200 (0x04B0) is factory default
			BYTE lo=0xB0;
			BYTE hi=0x04;
		...
		case IDC_RotateUnit:
			...
			//0 = 0deg or no rotation (trackball below buttons) factory default
			//1 = 90deg rotation (trackball on the left of buttons)
			//2 = 180deg rotation (trackball on top of buttons)
			//3 = 270deg rotation (trackball on right of buttons)
			if (hDevice == -1) return TRUE;
			...
			return TRUE;
		case IDC_NativeMouse:
			//Use this command to enable (factory default) or disable the native mouse
			if (hDevice == -1) return TRUE;
			...
			buffer[0]=0;
			buffer[1]=173; //0xad
			buffer[2]=1; //0=off, 1=on (factory default)
			...
			return TRUE;
		case IDC_MouseReport:
			//Use this command to enable or disable (factory default) the mouse data on the General Incoming Data report
			if (hDevice == -1) return TRUE;
			...
			return TRUE;
		case IDC_GeneralData:
			//Use this command to enable (factory default) or disable the General Incoming Data report
			if (hDevice == -1) return TRUE;
...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		productname="Found Device: XBA-Trackball Module, PID="+pidstring;
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	if (pData[2]<4) //General Incoming Data
	{
		...
		//Shuttle Analog
		hList = GetDlgItem(hDialog, IDC_ShuttleA);
		char msg[100];
		char str[80];
		_itoa_s(pData[14],msg,10);
		strcpy_s (str,"Shuttle Analog: ");
		strcat_s (str,msg);
		SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)str);

		//Jog Raw
		hList = GetDlgItem(hDialog, IDC_JogR);
		//char msg[100];
		//char str[80];
		_itoa_s(((pData[15]*256)+pData[16]),msg,10);
		strcpy_s (str,"Jog Raw: ");
		strcat_s (str,msg);
		SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)str);
		...
		int maxrows=3; //constant, 8 bits per byte
		...
		for (int i=3;i<5;i++) //4th and 5th bit of this column are the trackball side buttons
		{
			int temp1=pow(2.0,i); 
			BYTE temp2=(pData[6]&temp1); //check using bitwise AND the current value of this bit
			BYTE temp3=(lastpData[6]&temp1); //check using bitwise AND the previous value of this bit
			int state=0; //0=was up and is up, 1=was up and is down, 2= was down and is down, 3=was down and is up 
			if (temp2 != 0 && temp3 == 0) state = 1; //press
			else if (temp2 != 0 && temp3 != 0) state = 2; //held down
			else if (temp2 == 0 && temp3 != 0) state = 3; //release
			...
			case 1: //key was up and now is pressed
				if (i==3) strcat_s (strb,"Right, ");
				else strcat_s (strb,"Left, ");
				SendMessage(hList, WM_SETTEXT, 0 , (LPARAM)strb);
				break;
			case 2:
				if (i==3) strcat_s (strb,"Right, ");
		...
		char mouse[80];
		strcpy_s(mouse, "X: ");
		int xmouse=pData[8]+(256*pData[9]);
		_itoa_s(xmouse,mouse,10);
		hList = GetDlgItem(hDialog, IDC_TrackballX);
		SendMessage(hList, WM_SETTEXT, 0 , (LPARAM)mouse);

		strcpy_s(mouse, "Y: ");
		int ymouse=pData[10]+(256*pData[11]);
		_itoa_s(ymouse,mouse,10);
		hList = GetDlgItem(hDialog, IDC_TrackballY);
		SendMessage(hList, WM_SETTEXT, 0 , (LPARAM)mouse);
	...
	else if (pData[2] == 167) //0xA7 backlight LED state request
    {
		...
		//pData[9]=bank 2 blue
		//pData[10]=flash bank 1 //inverted 255=no flash, 0=flash
		//pData[11]=flash bank 2 //inverted 255=no flash, 0=flash

...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		productname="Found Device: XBA-Handwheel Module, PID="+pidstring;
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	if (pData[2]<4) //General Incoming Data
	{
		...
		//Handwheel Digital
		hList = GetDlgItem(hDialog, IDC_HandwheelD);
		if ((pData[7] & 0x40)!=0)
		{
			char msg[100]="Handwheel Digital: Clockwise";
			SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)msg);
		}
		if ((pData[7] & 0x80)!=0)
		{
			char msg[100]="Handwheel Digital: Counterclockwise";
			SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)msg);
		}

		//Handwheel Analog
		hList = GetDlgItem(hDialog, IDC_HandwheelA);
		if (pData[13] == 1) //cw
        {
            char msg[100]="Handwheel Analog: Clockwise";
			SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)msg);
        }
        else if (pData[13] == 255)
        {
           char msg[100]="Handwheel Analog: Counterclockwise";
			SendMessage(hList, WM_SETTEXT,NULL , (LPARAM)msg);
        }

		hList = GetDlgItem(hDialog, IDC_HandwheelR);
		if (hList == NULL) return TRUE;
		char msg[100];
		char str[80];
		_itoa_s(((pData[15]*256)+pData[16]),msg,10);
		strcpy_s (str,"Handwheel Raw: ");
		strcat_s (str,msg);

	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				if (pid==1227)
				{
					AddDevices(hDialog, "Found Device: XK-128, PID=1227 (PID #1)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1228)
				{
					AddDevices(hDialog, "Found Device: XK-128, PID=1228 (PID #2)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1229)
				{
					AddDevices(hDialog, "Found Device: XK-128, PID=1229 (PID #3)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1230)
				{
					AddDevices(hDialog, "Found Device: XK-128, PID=1230 (PID #4)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else

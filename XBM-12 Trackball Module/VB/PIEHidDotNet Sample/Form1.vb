...
Public Class Form1
    ...
    Dim lastdatageneral As Byte() = Nothing
    Dim lastdatamouse As Byte() = Nothing
    Dim lastdatalaser As Byte() = Nothing
    Dim dontchange As Boolean = False
    ...
    Public Sub HandlePIEHidData(ByVal data() As Byte, ByVal sourceDevice As PIEHid32Net.PIEDevice, ByVal perror As Integer) Implements PIEHid32Net.PIEDataHandler.HandlePIEHidData
        ...
        If sourceDevice.Pid = devices(selecteddevice).Pid Then
            ...
            If (data(2) < 4) Then 'General Incoming Data
                ...
                'Mouse motion if enabled
                c = lblX
                Dim xmotion As Integer = data(9) + 256 * data(10)
                Dim xmotions As Integer = BitConverter.ToInt16(New Byte() {data(9), data(10)}, 0) 'convert to a signed int
                SetText("X: " + xmotions.ToString())

                c = lblY
                Dim ymotion As Integer = data(11) + 256 * data(12)
                Dim ymotions As Integer = BitConverter.ToInt16(New Byte() {data(11), data(12)}, 0) 'convert to a signed int
                SetText("Y: " + ymotions.ToString())
        ...
        Else
            ...
            For i As Integer = 0 To devices.Length - 1
                ...
                Else
                    ...
                    ElseIf (devices(i).Pid = 1495) Then 'Device 7 Keyboard only endpoint
    ...
    Private Sub rbGIOn_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbGIOn.CheckedChanged
        'Turn on input reports
        If (dontchange = True) Then
            dontchange = False
            Return
        End If
        If selecteddevice <> -1 Then 'do nothing if not enumerated
    ...
    Private Sub rbGIOff_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbGIOff.CheckedChanged
        'Turn on input reports
        If (dontchange = True) Then
            dontchange = False
            Return
    ...
    Private Sub btnSetResolution_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSetResolution.Click

        If cboResolution.SelectedIndex <> -1 Then
            If selecteddevice <> -1 Then
                ...
                wdata(0) = 0
                wdata(1) = 168 '&HBB
                wdata(2) = cboResolution.SelectedIndex + 1 'Index of desired resolution
    ...
    Private Sub rbNativeMouseOn_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbNativeMouseOn.CheckedChanged
        'Turn on native mouse (which is trackball motion)
        If (dontchange = True) Then
            dontchange = False
            Return
        End If
        If selecteddevice <> -1 Then 'do nothing if not enumerated
            ...
            If result <> 0 Then
                LblStatus.Text = "Write Fail: " & result
            Else
                LblStatus.Text = "Write Success - Native mouse on"
            End If
        End If
    End Sub

    Private Sub rbNativeMouseOff_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles rbNativeMouseOff.CheckedChanged
        ...
        If selecteddevice <> -1 Then 'do nothing if not enumerated
            ...
            If result <> 0 Then
                LblStatus.Text = "Write Fail: " & result
            Else
                LblStatus.Text = "Write Success - Native mouse off"
            End If
        End If
    End Sub
    ...
    Private Sub btnRotate_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnRotate.Click
        If selecteddevice <> -1 Then 'do nothing if not enumerated
            ...
            wdata(1) = 161 '&HA1
            wdata(2) = cboRotate.SelectedIndex '0 = 0 deg = trackball down (factory default), 1 = 90 deg = trackball left, 2 = 180 deg = trackball up, 3 = 270 deg = trackball right
            ...
            If result <> 0 Then
                LblStatus.Text = "Write Fail: " & result
            Else
                LblStatus.Text = "Write Success - Rotate"
            End If
        End If
    End Sub
    ...
    Private Sub btnClickLockDelay_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClickLockDelay.Click
        If selecteddevice <> -1 Then 'do nothing if not enumerated
            ...
            If result <> 0 Then
                LblStatus.Text = "Write Fail: " & result
            Else
                LblStatus.Text = "Write Success - ClickLock Delay"
            End If
        End If
    End Sub

    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        public void HandlePIEHidData(Byte[] data, PIEDevice sourceDevice, int error)
        {
            ...
            if (sourceDevice == devices[selecteddevice])
            {
                ...
                if (data[2] < 4)
                {
                    ...
                    c = lblX;
                    int xmotion = data[8] + 256 * data[9]; //0-0xFFFF
                    int xmotions = BitConverter.ToInt16(new byte[] { data[8], data[9] }, 0); //convert to a signed int
                    SetText("X: " + xmotions.ToString());

                    c = lblY;
                    int ymotion = data[10] + 256 * data[11];
                    int ymotions = BitConverter.ToInt16(new byte[] { data[10], data[11] }, 0); //convert to a signed int
                ...
                else if (data[2] == 167) //0xA7 backlight LED state request
                {
                    ...
                    this.SetListBox("Flash Bank 1 =" + (data[10])); //inverted 255=no flash, 0=flash
                    //bytes 7, 8, 9 for bank 2
                    ...
                    this.SetListBox("Flash Bank 2=" + (data[11])); //inverted 255=no flash, 0=flash
                    //byte 11 for bank 2
        ...
        private void BtnEnumerate_Click(object sender, EventArgs e)
        {
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    ...
                    else
                    {
                        ...
                        else if (devices[i].Pid == 1495)
                        {
                            ...
                            CboDevices.Items.Add("XBA-Trackball Module (" + devices[i].Pid + "=PID #5), ID: " + i);
                            cbotodevice[cbocount] = i;
                            cbocount++;
                            cboPIDs.SelectedIndex = 4;
            ...
            if (CboDevices.Items.Count > 0)
            {
                ...
                selecteddevice = cbotodevice[CboDevices.SelectedIndex];
                if (devices[selecteddevice].ReadLength > 20)
                {
        ...
        {
            //RGB Global Intensity - this command is meant to be used once the banks of LEDs have been configured to the desired colors. 255 is the brightest and 1 is the dimmest.
            if (CboDevices.SelectedIndex != -1) //do nothing if not enumerated
            {
                for (int j = 0; j < devices[selecteddevice].WriteLength; j++)
                {
                ...
                {
                    toolStripStatusLabel1.Text = "Write Success - RGB Global Intensity";
                }
            }
        }
        ...
        private void chkLED1_CheckStateChanged_1(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1)
            {
                ...
                wData[1] = 179; //0xb3
                wData[2] = ledindex; //0=Pin 1, 1=Pin 2, 2=Pin 3, 3=Pin 4

...
//	BYTE buffer[80];  //this globally declared only to keep track of LEDs	
    ...
    case WM_COMMAND:
		...
		case IDC_SetFader1:
			...
			buffer[0]=0;
			buffer[1]=149; //0x95
			buffer[2]=0; //fader index; 0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4
		...
		case IDC_SetFader2:
			...
			buffer[0]=0;
			buffer[1]=149; //0x95
			buffer[2]=1; //fader index; 0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4
		...
		case IDC_SetFader3:
			...
			buffer[0]=0;
			buffer[1]=149; //0x95
			buffer[2]=2; //fader index; 0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4
		...
		case IDC_SetFader4:
			...
			buffer[0]=0;
			buffer[1]=149; //0x95
			buffer[2]=3; //fader index; 0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4
		...
		case IDC_SetFaderAll:
			...
			hList = GetDlgItem(hDialog, IDC_TxtFader1);
			SendMessage(hList, WM_GETTEXT, 8, (LPARAM)val);
			buffer[2]=atoi(val); //desired value of fader 1
			
			hList = GetDlgItem(hDialog, IDC_TxtFader2);
			SendMessage(hList, WM_GETTEXT, 8, (LPARAM)val);
			buffer[3]=atoi(val); //desired value of fader 2 

			hList = GetDlgItem(hDialog, IDC_TxtFader3);
			SendMessage(hList, WM_GETTEXT, 8, (LPARAM)val);
			buffer[4]=atoi(val); //desired value of fader 3
		...
		case IDC_EnableTouchStop:
			...
			buffer[0]=0;
			buffer[1]=161; //0xA1
			buffer[2] = 255; //0=fader 1, 1=fader 2, 2=fader 3, 3=fader 4 OR 255 to set all
		...
		case IDC_DESCRIPTOR:
			...
			if (result ==0 && buffer[2]==214)
			{
				...
				strcpy_s (str,"LEDs=");
				if (buffer[10]&1) strcat_s (str,"LED 1 ");
				if (buffer[10]&2) strcat_s (str,"LED 2 ");
				if (buffer[10]&4) strcat_s (str,"LED 3 ");
				if (buffer[10]&8) strcat_s (str,"LED 4 ");
				if (strlen(str)==5) strcat_s (str,"None ");
				AddEventMsg(hDialog, str);
...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		productname="Found Device: XBA-Fader Module, PID="+pidstring;
...
DWORD __stdcall HandleDataEvent(UCHAR *pData, DWORD deviceID, DWORD error)
{
	...
	{
		//Fader 1 = (data[3] * 256 + data[4]);
		//Fader 2 = (data[5] * 256 + data[6]);
		//Fader 3 = (data[7] * 256 + data[8]);
		//Fader 4 = (data[9] * 256 + data[10]);
	}
	else if (pData[2]==173) //get voltage
	{
		//voltage = data[6]+ (data[7]/10.0f);
		long long voltagewhole=pData[6];
		std::string thisvoltage;
		thisvoltage = std::to_string(voltagewhole);
		long long voltagedecimal=pData[7];

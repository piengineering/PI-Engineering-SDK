...
void FindAndStart(HWND hDialog)
{
	...
	for (long i=0; i<count; i++)    {
		...
		{	
			...
			else    
			{
				if (pid==1275)
				{
					AddDevices(hDialog, "Found Device: XKE-124 T-bar, PID=1275 (PID #1)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1276)
				{
					AddDevices(hDialog, "Found Device: XKE-124 T-bar, PID=1276 (PID #2)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1277)
				{
					AddDevices(hDialog, "Found Device: XKE-124 T-bar, PID=1277(PID #3)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else if (pid==1278)
				{
					AddDevices(hDialog, "Found Device: XKE-124 T-bar, PID=1278 (PID #4)");
					combotodevice[cbocount] = i; //this is the handle
					cbocount++;
				}
				else
	...
	{
		AddDevices(hDialog, "No X-keys XKE-124 T-bar devices found");
	}
}

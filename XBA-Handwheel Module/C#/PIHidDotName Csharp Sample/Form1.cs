    ...
    public partial class Form1 : Form, PIEDataHandler, PIEErrorHandler
    {
        ...
        public void HandlePIEHidData(Byte[] data, PIEDevice sourceDevice, int error)
        {
            ...
            if (sourceDevice == devices[selecteddevice])
            {
                ...
                {
                    //check the keyboard state byte (keyboard endpoint required)
                    ...
                    int maxrows = 3; //number of rows of Xkeys digital button data, this device has GPIO inputs on 4th row
                    c = this.LblButtons;
                    ...
                    //Handwheel Digital
                    byte val = (byte)(data[7] & 0x40);
                    if (val != 0)
                    {
                        c = LblJogD;
                        SetText("Handwheel Digital: Clockwise");
                    }
                    val = (byte)(data[7] & 0x80);
                    if (val != 0)
                    {
                        c = LblJogD;
                        SetText("Handwheel Digital: Counterclockwise");
                    }
                    ...
                    {
                        c = this.LblJog;
                        this.SetText("Handwheel Analog: Clockwise");
                    }
                    else if (data[13] == 255)
                    {
                        c = this.LblJog;
                        this.SetText("Handwheel Analog: Counterclockwise");
                    }

                    //Handwheel Raw Count
                    c = this.lblJogRaw;
                    this.SetText("Handwheel Raw Count: " + (data[15] * 256 + data[16]).ToString());
        ...
        private void BtnDescriptor_Click(object sender, EventArgs e)
        {
            ...
            if (CboDevices.SelectedIndex != -1)
            {
                ...
                if ((byte)(data[10] & 16) != 0) pinson = pinson + "GPIO 1, ";
                if ((byte)(data[10] & 32) != 0) pinson = pinson + "GPIO 2, ";
                if ((byte)(data[10] & 64) != 0) pinson = pinson + "GPIO 3, ";
                if ((byte)(data[10] & 128) != 0) pinson = pinson + "GPIO 4, ";
